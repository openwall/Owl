 ...
 * See LICENSE
 *
 * $Owl: Owl/packages/msulogin/msulogin/sulogin.c,v 1.7 2012/08/15 06:43:31 solar Exp $
 */
...
static int sulogin(void)
{
	...
	struct termios s, t;
	char user[LINE_BUFFER_SIZE], pass[LINE_BUFFER_SIZE];
	struct passwd *pw;
	...
	while (1) {
		putline_fd("\nLogin as a root user for maintenance"
		    " or press Ctrl-D for normal startup.\n\n", out);
		putline_fd("login [root]: ", out);
		if ((retval = getline_fd(user, sizeof(user), in)) < 0)
			state = -retval;
		if (state != S_WORKING)
			break;
...
int main(int argc, char **argv)
{
	...
	if (geteuid() != 0) {
		putline_fd("Must be started as root.\n", STDERR_FILENO);
		return S_FAILED;
	}
